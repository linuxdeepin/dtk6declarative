...
void DConfigWrapper::setValue(const QString &key, const QVariant &value)
{
    ...
    QMetaObject::invokeMethod(impl.get(), &DConfig::setValue, key, value);
}
...
void DConfigWrapper::resetValue(const QString &key)
{
    ...
    QMetaObject::invokeMethod(impl.get(), &DConfig::reset, key);
}
...
void DConfigWrapper::componentComplete()
{
    ...
    QMetaObject::invokeMethod(configThread, [this] {
...
auto callInGuiThread(DConfigWrapper *wrapper, Fun fun, Args&&... args) {
...
void DConfigWrapper::initializeProperties()
{
    ...
    const DefalutProperties &properties = callInGuiThread(this, propertyAndValues, this);
    ...
    for (auto iter = properties.begin(); iter != properties.end(); iter++) {
        ...
        const auto value = impl->value(iter.key(), iter.value());
        callInGuiThread(this, [this, iter, value] {
            mo->setValue(iter.key(), value);
        });
    }

    // Using QueuedConnection because impl->setValue maybe emit sync signal in `propertyWriteValue`.
    connect(impl.get(), &DTK_CORE_NAMESPACE::DConfig::valueChanged, this, [this, properties](const QString &key) {
        ...
        if (properties.contains(proName)) {
            ...
            const auto value = impl->value(proName, properties.value(proName));
            const bool isDefault = impl->isDefaultValue(proName);
            callInGuiThread(this, [this, proName, value, isDefault] {
                if (isDefault) {
                    this->nonDefaultValueKeyList.removeOne(proName);
                } else if (!this->nonDefaultValueKeyList.contains(proName)) {
                    this->nonDefaultValueKeyList.append(proName);
                }
                mo->setValue(proName, value);
            });
        }
        QMetaObject::invokeMethod(this, &DConfigWrapper::valueChanged, key);
    }, Qt::DirectConnection);
}
